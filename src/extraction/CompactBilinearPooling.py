...
class CompactBilinearPooling(nn.Module):
    ...
    def forward(self, bottom1, bottom2):
        ...
        """
        # If inputs are 2D embeddings: [batch_size, input_dim]
        if bottom1.dim() == 2 and bottom2.dim() == 2:
            sketch_1 = bottom1.mm(self.sparse_sketch_matrix1)
            sketch_2 = bottom2.mm(self.sparse_sketch_matrix2)
            fft1 = afft.fft(sketch_1)
            fft2 = afft.fft(sketch_2)
            fft_product = fft1 * fft2
            cbp = afft.ifft(fft_product).real
            return cbp
        
        # If inputs are 1D embeddings: [input_dim]
        if bottom1.dim() == 1 and bottom2.dim() == 1:
            sketch_1 = bottom1 @ self.sparse_sketch_matrix1
            sketch_2 = bottom2 @ self.sparse_sketch_matrix2
            fft1 = afft.fft(sketch_1)
            fft2 = afft.fft(sketch_2)
            fft_product = fft1 * fft2
            cbp = afft.ifft(fft_product).real
...
if __name__ == '__main__':
    ...
    bottom1 = Variable(torch.randn(512))
    bottom2 = Variable(torch.randn(512))
